...
export async function buildIssuerEntityConfiguration_V1_4(
  metadata: {
    iss: string;
    trust: Config["trust"];
...
export async function buildMockMdlMdoc_V1_4(
  expiration: Date,
...
export async function buildMockSdJwt_V1_4(
  metadata: {
    iss: string;
    trust: Config["trust"];
    trustAnchor: Config["trust_anchor"];
  ...
  const issEntityConfiguration = await buildIssuerEntityConfiguration_V1_4(
    metadata,
    keyPair,
  );
