...
/**
 * Reads JWKS from a file. Throws if the file does not exist or contains invalid data.
 * Use this in test flows where key material is expected to be pre-generated by `wct init`.
 * @param jwksPath The directory path where JWKS files are stored.
 * @param filename The name of the JWKS file to read.
 * @returns The loaded KeyPair.
 * @throws If the file is missing or cannot be parsed as valid JSON.
 */
export function readJwks(jwksPath: string, filename: string): KeyPair {
  const filePath = `${jwksPath}/${filename}`;
  let data: string;
  try {
    data = readFileSync(filePath, "utf-8");
  } catch {
    throw new Error(
      `Key file not found at '${filePath}'. Run 'wct init' to generate the required cryptographic artifacts.`,
    );
  }
  try {
    ...
    throw new Error(
      `Key file at '${filePath}' contains invalid data. Run 'wct init --force' to regenerate.`,
    );
  }
}
