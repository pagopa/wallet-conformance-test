...
/**
 * Reads a certificate from disk. Throws if the file is missing, invalid, or expired.
 * Use this in test flows where certificates are expected to be pre-generated by `wct init`.
 * @param certPath The directory path where certificate files are stored.
 * @param filename The name of the certificate file to read.
 * @returns The base64-DER encoded certificate string.
 * @throws If the file is missing, cannot be parsed, or has expired.
 */
export function readCertificate(certPath: string, filename: string): string {
  const filePath = `${certPath}/${filename}`;
  let certPem: string;
  try {
    certPem = readFileSync(filePath, "utf-8");
  } catch {
    throw new Error(
      `Certificate file not found at '${filePath}'. Run 'wct init' to generate the required cryptographic artifacts.`,
    );
  }
  let certDerBase64: string;
  try {
    certDerBase64 = pemToBase64Der(certPem);
  } catch {
    throw new Error(
      `Certificate file at '${filePath}' contains invalid data. Run 'wct init --force' to regenerate.`,
    );
  }
  if (hasX509CertificateExpired(certDerBase64))
    throw new CertificateExpiredError(
      `Certificate at '${filePath}' has expired. Run 'wct init --force' to regenerate.`,
    );
  return certDerBase64;
}
