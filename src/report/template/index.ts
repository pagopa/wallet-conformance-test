import type { ReportData, ReportView } from "@/report/template/types";
...
export function renderHtml(
  ...
  config?: Config,
  view: ReportView = "both",
): string {
  const data = buildReportData(session, config);

  if (view === "executive") return renderSingleView("executive", data);
  if (view === "technical") return renderSingleView("technical", data);

  return renderBothViews(data);
}

// ─── Private: layout composers ────────────────────────────────────────────────
...
function renderBothViews(data: ReportData): string {
  return wrapDocument(
    `
  <!-- Screen view switcher (hidden in print) -->
  ...
  ${renderExecutiveView(data, true)}
  ${renderTechnicalView(data, true)}
...
function renderSingleView(
  ...
  const content =
    view === "executive"
      ? renderExecutiveView(data, false)
      : renderTechnicalView(data, false);

  // Single-view: unwrap from .view-panel so it's always visible without JS.
  const unwrapped = content
    .replace(/^\s*<div[^>]*class="view-panel[^"]*"[^>]*>\s*/m, "")
    .replace(/\s*<\/div>\s*$/m, "");

  return wrapDocument(unwrapped);
}
