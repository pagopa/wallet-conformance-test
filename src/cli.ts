...
function setEnvFromOptions(options: CliOptions): NodeJS.ProcessEnv {
  ...
  if (options.presentationQRId) {
    env.CONFIG_PRESENTATION_QR_ID = options.presentationQRId;
  }
...
function addCommonOptions(command: Command): Command {
  return command
    ...
    .option(
      "--presentation-qr-id <names>",
      "ID or Class of the HTML container encapsulating the QR Code (env: CONFIG_PRESENTATION_QR_ID)",
    )
    .option(
