  ...
  describe(`[${testConfig.name}] Status List Tests`, () => {
    ...
    beforeAll(async () => {
      ...
      if (!ioWalletSdkConfig.isVersion(ItWalletSpecsVersion.V1_0)) {
        // Find the first SD-JWT credential with a status_list claim
        for (const credObj of credentialResponse.response?.credentials ?? []) {
          try {
            ...
            // non-SD-JWT or malformed — skip silently
          }
        }
        ...
        if (statusListUri) {
          ...
          if (statusListJwt) {
            try {
              ...
              // decode / decompress errors surface as individual test failures
            }
          }
        }
      }
    ...
    test("CI_175: OAuth Status List Support | Tool queries the Status List endpoint using uri from status.status_list", async ({
      skip,
    }) => {
      ...
      try {
        if (ioWalletSdkConfig.isVersion(ItWalletSpecsVersion.V1_0)) skip();
        if (!statusListUri || statusListResponseStatus === undefined) skip();
    ...
    test("CI_176: Digital Credential Index Allocation | idx in the credential is a valid position in the Status List byte array", async ({
      skip,
    }) => {
      ...
      try {
        if (ioWalletSdkConfig.isVersion(ItWalletSpecsVersion.V1_0)) skip();
        if (credentialIdx === undefined || !decompressedList) skip();
        ...
        expect(typeof statusAtIdx, "Status value at idx MUST be a number").toBe(
          "number",
        );
    ...
    test("CI_177: Status List Token Cryptographic Format | Status List Token is a signed JWT with typ=statuslist+jwt", async ({
      skip,
    }) => {
      ...
      log.start("Conformance test: Status List Token JWT cryptographic format");

      let testSuccess = false;
      try {
        if (ioWalletSdkConfig.isVersion(ItWalletSpecsVersion.V1_0)) skip();
        if (!statusListJwt) skip();
    ...
    test("CI_178: Status List Bit Configuration | bits per credential entry is one of {1, 2, 4, 8}", async ({
      skip,
    }) => {
      ...
      try {
        if (ioWalletSdkConfig.isVersion(ItWalletSpecsVersion.V1_0)) skip();
        if (statusListBits === undefined) skip();
    ...
    test("CI_179: Status List Byte Array | byte array is non-empty and the credential index is assigned", async ({
      skip,
    }) => {
      ...
      try {
        if (ioWalletSdkConfig.isVersion(ItWalletSpecsVersion.V1_0)) skip();
        if (!statusListLst || !decompressedList || credentialIdx === undefined)
          skip();
    ...
    test("CI_180: Status List Status Values Setting | status at credential idx is VALID (0x00)", async ({
      skip,
    }) => {
      ...
      try {
        if (ioWalletSdkConfig.isVersion(ItWalletSpecsVersion.V1_0)) skip();
        if (!decompressedList || credentialIdx === undefined) skip();
    ...
    test("CI_181: Status List Byte Array Compression | lst is DEFLATE/ZLIB compressed (RFC 1951/RFC 1950)", async ({
      skip,
    }) => {
      ...
      try {
        if (ioWalletSdkConfig.isVersion(ItWalletSpecsVersion.V1_0)) skip();
        if (!statusListLst || statusListBits === undefined) skip();
    ...
    test("CI_181a: Recommended Compression Level | decompressed data is valid and compression is present", async ({
      skip,
    }) => {
      ...
      try {
        if (ioWalletSdkConfig.isVersion(ItWalletSpecsVersion.V1_0)) skip();
        if (!statusListLst || !decompressedList || statusListBits === undefined)
          skip();
        ...
        expect(lst.length, "Compressed lst MUST be present").toBeGreaterThan(0);
    ...
    test("CI_182: Status List Endpoint Availability | endpoint returns HTTP 2xx", async ({
      skip,
    }) => {
    ...
    test("CI_183: Status List Status Values Definition | status values are within the spec-defined range", async ({
      skip,
    }) => {
      ...
      try {
        ...
        const isSpecDefined = specDefinedValues.includes(statusValue as number);
        log.debug(
    ...
    test("CI_184: Status List Optional Additional States | additional state values do not exceed the bits-configured range", async ({
      skip,
    }) => {
    ...
    test("CI_185: Status List Token Parameters | token contains required compliance-table claims (iss, sub, iat, status_list)", async ({
      skip,
    }) => {
      ...
      try {
        ...
        expect(typeof slClaim?.bits, "status_list.bits MUST be a number").toBe(
          "number",
        );
    ...
    test("CI_186: Recommended Status List Token Expiration | exp does not exceed iat + 86400 (24 h)", async ({
      skip,
    }) => {
    ...
    test("CI_187: JSON-Encoded Status List Structure | status_list claim matches the compliance table", async ({
      skip,
    }) => {
      ...
      try {
        if (ioWalletSdkConfig.isVersion(ItWalletSpecsVersion.V1_0)) skip();
        if (statusListBits === undefined || !statusListLst) skip();

        const bits = statusListBits as number;
        const lst = statusListLst as string;

        log.debug(`→ status_list.bits: ${bits}`);
        log.debug(`→ status_list.lst (first 20 chars): ${lst.slice(0, 20)}...`);
        ...
        expect(lst.length, "status_list.lst MUST not be empty").toBeGreaterThan(
          0,
        );
    ...
    test("CI_190: Status List Claim JSON Object Parameters | status.status_list in credential has valid idx and uri", async ({
      skip,
    }) => {
      ...
      try {
        if (ioWalletSdkConfig.isVersion(ItWalletSpecsVersion.V1_0)) skip();
        if (credentialIdx === undefined || !statusListUri) skip();
        ...
        expect(uri.length, "status_list.uri MUST not be empty").toBeGreaterThan(
          0,
        );
        expect(
          () => new URL(uri),
    ...
    test("CI_191: Status List Endpoint Successful Response | HTTP 2xx and Content-Type: application/statuslist+jwt", async ({
      skip,
    }) => {
    ...
    test("CI_192: HTTP Status List Response Gzip Content-Encoding | response uses Content-Encoding: gzip per RFC 9110", async ({
      skip,
    }) => {
      ...
      log.start("Conformance test: Status List response gzip Content-Encoding");

      let testSuccess = false;
      try {
        if (ioWalletSdkConfig.isVersion(ItWalletSpecsVersion.V1_0)) skip();
        if (!statusListUri) skip();
