...
testConfigs.forEach((testConfig) => {
  describe(`[${testConfig.name}] Credential Request Validation`, () => {
    ...
    // -----------------------------------------------------------------------
    // CI_080 — Fresh Key Generation
    // -----------------------------------------------------------------------

    test("CI_080: Fresh Key Generation | Issuer handles repeated credential requests with the same proof key", async () => {
      const log = baseLog.withTag("CI_080");
      const DESCRIPTION =
        "Issuer correctly handled repeated credential requests with same proof key (SHOULD-level)";

      log.start("Conformance test: Verifying fresh key generation enforcement");

      let testSuccess = false;
      try {
        log.debug(
          "→ Generating a fixed key pair for both credential requests...",
        );
        ...
        const StepWithFixedSigner = withCredentialRequestOverrides(
          withCredentialSignJwtOverride(
            testConfig.credentialRequestStepClass,
            signJwtCallback([fixedPrivateKey]),
          ),
          {
            signer: {
              alg: "ES256",
              method: "jwk" as const,
              publicJwk: fixedPublicKey,
            },
          },
        );

        log.debug("→ Sending first credential request with fixed key...");
        const firstResult = await runCredentialStep(StepWithFixedSigner);
        log.debug(
          `  First request result: ${firstResult.success ? "success" : "failed"}`,
        );

        log.debug(
          "→ Sending second credential request with the same fixed key...",
        );
        const secondResult = await runCredentialStep(StepWithFixedSigner);
        log.debug(
        ...
        if (!secondResult.success) {
          log.debug(
            "  ✅ Issuer correctly rejected reuse of the same proof key (SHOULD-level requirement enforced)",
          );
        } else {
          log.debug(
            "  ⚠️  Issuer accepted reuse of the same proof key (SHOULD-level requirement not enforced — informational)",
          );
        }

        // CI_080 is a SHOULD-level requirement: assert the first request succeeded
        // (proving the test infrastructure worked) without mandating the second fails.
