...
- **CI_079**: Runs a successful credential request and inspects the issued credential for a `status` claim (supporting both SD-JWT VC and mdoc-CBOR formats). For spec version 1.3, verifies that `status.status_list` is present with an integer `idx` and a string `uri`. For spec version 1.0, verifies that `status.status_assertion` is present with a `credential_hash_alg` string. Confirms the credential references a valid revocation/status mechanism.
